...
==Beim Starten eines Linux-Kernels können diesem (wie einem Anwendungsprogramm) Kommandozeilen-Parameter übergeben werden.== Über diese _kernel command line_ kann man zahlreiche Parameter setzen.

Eine vollständige Liste finden Sie in der [Linux-Kernel-Dokumentation](https://www.kernel.org/doc/html/latest/admin-guide/kernel-parameters.html). Dem Kernel nicht bekannte Parameter werden an den init-Prozess übergeben. Beispiele für oft verwendete Parameter sind:

|   |   |
|---|---|
|`root=`|Gerät, auf dem das _root file system_ liegt|
|`ro`|_root file system_ wird anfangs _read-only_ eingehängt|
|`init=`|Programm, das als PID 1 gestartet wird (default: `/sbin/init`)|
|`loglevel=`|Umfang des Loggings auf der Konsole|
|`quiet`|Zeige nur wenig Logging-Nachrichten|
|`(no)splash`|zeige eine grafische _splash screen_ statt der Konsole|
|`single`|im _single user mode_ zur Fehlerbehebung starten|
|`apparmor=`|Das Sicherheits-Feature AppArmor (de)aktivieren|

Normalerweise startet ein [Boot Loader](https://moodle.oncampus.de/modules/ir866/onmod/admin/boot.html#h_boot-loader) den Kernel und übergibt dabei die _kernel command line_. Daher konfiguriert man diese Parameter in den Konfigurationsdateien des Boot Loaders.

/proc/cmdline 中放了 _kernel command line Parameter_  Ihres Ubuntu-Systems
Zur Laufzeit kann man die _kernel command line_ mit `cat /proc/cmdline` abfragen.

```
yzh@BNB23350:~$ cat /proc/cmdline
...
这些Parameter, kernel 会去使用它们

Der Kernel hat einige interne Parameter, die von privilegierten Prozessen zur Laufzeit modifiziert werden können. Sie sind im Pseudo-Dateisystem `/proc/sys` zugänglich. Man kann sie dort direkt lesen und schreiben. Meistens verwendet man dafür stattdessen das benutzerfreundlichere Kommando [sysctl(8)](https://man7.org/linux/man-pages/man8/sysctl.8.html).


Lassen Sie sich mit `sysctl -a` eine Liste aller Laufzeit-Parameter Ihres Systems mit den aktuellen Werten anzeigen.


Die Parameter-Namen sind hierarchisch aufgebaut; die erste Komponente ist eine grobe Kategorie, z.B.
- `fs.*` : _file system_
- `net.*` : Netzwerk-Stack
- `vm.*` : _virtual memory_, also Speicherverwaltung

Zwei typische Beispiele sind:
- `vm.swappiness` — legt fest, wie intensiv [Swap Space](https://moodle.oncampus.de/modules/ir866/onmod/mem/swap.html) genutzt wird (1 = möglichst selten, 100 = sehr oft, Default = 60)
- `net.ipv6.conf.all.disable_ipv6 = 1` deaktiviert die IPv6-Unterstützung.


---

Die Dokumentation der verschiedenen Parameter findet man verteilt in den entsprechenden Abschnitten der [Linux-Kernel-Dokumentation](https://www.kernel.org/doc/html/latest/); suchen Sie nach dem Stichwort _sysctl_.

Sie können in Ubuntu die komplette Linux-Kernel-Dokumentation lokal installieren:  
`sudo apt install linux-doc`. Dann finden Sie alle Dokumente als Text im Markup-Format [reStructuredText](https://en.wikipedia.org/wiki/ReStructuredText) (`.rst`) unter `/usr/share/doc/linux-doc`.

## 2.1 ` /etc/sysctl.conf` und `etc/sysctl.d/


这些Parameter, kernel 会去使用它们

可以将 想要 Automatisches Setzen 的 Parametern 放到这里  /etc/sysctl.conf
**Automatisches Setzen von Parametern**: Über die Konfigurationsdatei `/etc/sysctl.conf` sowie Dateien im Verzeichnis `/etc/sysctl.d` können Laufzeit-Parameter beim Booten automatisch gesetzt werden, siehe [sysctl.conf(5)](https://man7.org/linux/man-pages/man5/sysctl.conf.5.html) und [sysctl.d(5)](https://man7.org/linux/man-pages/man5/sysctl.d.5.html).

```
yzh@BNB23350:~$ cat /etc/sysctl.conf
#
...
```
yzh@BNB23350:~$ ll /etc/sysctl.d
total 40
drwxr-xr-x  2 root root 4096 Nov 22  2023 ./
drwxr-xr-x 80 root root 4096 Dec 20 14:36 ../
...
lrwxrwxrwx  1 root root   14 Sep 19  2023 99-sysctl.conf -> ../sysctl.conf
...
fs.dentry-state = 22044 3286    45      0       1251    0
fs.dir-notify-enable = 1
fs.epoll.max_user_watches = 3608843
fs.fanotify.max_queued_events = 16384
fs.fanotify.max_user_groups = 128
fs.fanotify.max_user_marks = 134762
fs.file-max = 1619534
fs.file-nr = 480        0       1619534
fs.inode-nr = 20834     0
fs.inode-state = 20834  0       0       0       0       0       0
...
fs.nfs.nfs_congestion_kb = 128832
...
fscache.object_max_active = 20
fscache.operation_max_active = 10
kernel.acct = 4 2       30
...
kernel.domainname = ivu-ag.com
kernel.ftrace_dump_on_oops = 0
kernel.ftrace_enabled = 1
kernel.hostname = BNB23350
kernel.hotplug =
...
kernel.random.boot_id = be7df058-4737-4464-a5bb-91c03b2192f5
kernel.random.entropy_avail = 256
kernel.random.poolsize = 256
kernel.random.urandom_min_reseed_secs = 60
kernel.random.uuid = 9b28f367-a71b-40c9-aaef-633a0c12722d
...
net.ipv4.tcp_mem = 188790       251723  377580
...
net.ipv4.udp_mem = 377583       503447  755166
...
net.ipv4.vs.sync_qlen_max = 125879
...
net.sctp.sctp_mem = 377637      503516  755274
...
user.max_cgroup_namespaces = 63326
user.max_fanotify_groups = 128
user.max_fanotify_marks = 134762
user.max_inotify_instances = 128
user.max_inotify_watches = 524288
user.max_ipc_namespaces = 63326
user.max_mnt_namespaces = 63326
user.max_net_namespaces = 63326
user.max_pid_namespaces = 63326
user.max_time_namespaces = 63326
user.max_user_namespaces = 63326
user.max_uts_namespaces = 63326
...
# 3 Übung: Dynamische Parameter

1. Lassen Sie sich die _kernel command line_ Ihres Ubuntu-Systems anzeigen: `cat /proc/cmdline`. Was bedeuten die Parameter jeweils?
    
2. In der Konfigurationsdatei `/etc/default/grub` können unter anderem Kommandozeilen-Parameter definiert werden, die dann in die Grub-Konfiguration `/boot/grub/grub.cfg` generiert werden. Ersetzen Sie in `/etc/default/grub` die Optionen `quiet splash` durch `nosplash`, aktualisieren Sie mit `update-grub` die Grub-Konfiguration, und starten Sie das System neu. Welche Veränderung beobachten Sie? Stellen Sie dann den ursprünglichen Zustand wieder her.
    
3. Betrachten Sie `/etc/sysctl.conf` und die Dateien in `/etc/sysctl.d`. Welche Parameter werden gesetzt?
    
4. Setzen Sie manuell `vm.swappiness = 10`, damit Ihr System seltener den Swap Space nutzt. Machen Sie dann die Einstellung persistent, indem Sie eine entsprechende Datei in `/etc/sysctl.d` anlegen. Verifizieren Sie, dass der Parameter beim Neustart korrekt gesetzt wird.
